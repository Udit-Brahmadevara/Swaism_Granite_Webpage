/** Home: rotating hero of current lots, then a slice of the collection. */
...
if (frame) {
  frame.innerHTML = `
    <div class="carousel__frame">
      ${HERO.map((h, i) => `
        <div class="carousel__slide" data-active="${i === 0}" role="img"
             aria-label="${esc(h.stone.name)}" data-src="${esc(h.stone.img)}"></div>`).join('')}
      <div class="carousel__scrim"></div>
      <div class="carousel__caption">
        <div class="kicker" data-kicker>${esc(HERO[0].kicker)}</div>
        <div class="name" data-name>${esc(HERO[0].stone.name)}</div>
      </div>
      <div class="carousel__dots" role="tablist" aria-label="Featured stones">
        ${HERO.map((h, i) => `
          <button class="carousel__dot" type="button" role="tab" data-go="${i}"
                  aria-selected="${i === 0}" aria-label="Show ${esc(h.stone.name)}"><i></i></button>`).join('')}
      </div>
      <div class="carousel__nav">
        <button type="button" data-prev aria-label="Previous stone">&#8249;</button>
        <button type="button" data-next aria-label="Next stone">&#8250;</button>
      </div>
    </div>
  ...
  /* Five full-size slabs used to download before the page had painted — about
     700KB for four images nobody had seen yet. Load the first immediately and
     the rest only when the browser is idle, or sooner if the slide is reached.
     image-set() lets the browser take the WebP and fall back to the JPEG. */
  ...
  createCarousel(frame, {
    ...
    onChange: i => {
      ...
      name.textContent   = HERO[i].stone.name;
    },
  });
}
